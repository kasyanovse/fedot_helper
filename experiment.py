...
class Experiment:
    def __init__(self,
        ...
        """ Container for data and some parameters
    ...
    def get_data(self, validation: bool = True):
        holdout_splitter = DataSplitter(data=self.data, split_ratio=self.holdout_split_ratio, seed=self.seed,
...
class DataSplitter:
    def __init__(self, data: Data, split_ratio: Optional[float], seed: int,
                 cv_folds: Optional[int], forecast_length: Optional[int]):
        self.data = data
    ...
    def get(self):
        if self.data.ordered:
            ...
            validation_blocks = int(data_shape * test_share // self.forecast_length)
            horizon = self.forecast_length * validation_blocks

        indexes = np.arange(self.data.shape[0])
        if self.cv_folds is not None:
            if self.data.ordered:
                kf = TimeSeriesSplit(n_splits=self.cv_folds, test_size=horizon).split(indexes, indexes)
            else:
